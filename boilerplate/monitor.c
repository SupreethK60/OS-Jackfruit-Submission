 ...
 *
 * Implements a kernel-space memory monitor that:
 *   - Tracks container processes via ioctl registration
 *   - Maintains a linked list of monitored entries protected by mutex
 *   - Periodically checks RSS against soft and hard limits
 *   - Emits soft-limit warnings (once per entry) via printk
 *   - Enforces hard limits by sending SIGKILL
 *   - Cleans up stale entries for exited processes
 *
 * Authors:
 ...
 * soft-limit warning has been emitted, and list linkage.
 * ============================================================== */
 ...
 *
 * We use a mutex (not spinlock) because the ioctl handler runs
 * in process context where sleeping is allowed, and kmalloc with
 * GFP_KERNEL may sleep. The timer callback uses mutex_trylock
 * to avoid blocking in softirq-like context.
 * ============================================================== */
 ...
 *
 * TODO 3: Periodic monitoring implementation.
 * Iterates tracked entries, removes stale/exited processes,
 * emits soft-limit warnings (once), enforces hard limits.
 * Uses list_for_each_entry_safe for safe deletion during iteration.
 * --------------------------------------------------------------- */
